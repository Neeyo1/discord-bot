    ...
    try:
        ...
        if(int(date_today.hour) >= 10):
            if(len(ros_tanroth_count) >= 7):
                msg = "Możliwy Tanroth, " + str(len(ros_tanroth_count)) + " rosów online"
        ...
        if(len(ros_teza_count) >= players_online_to_ping):
            msg = "Możliwa Teza, " + str(len(ros_teza_count)) + " rosów online"
            await channel.send(content=msg)
        if(len(ros_magua_count) >= players_online_to_ping):
            msg = "Możliwy Magua, " + str(len(ros_magua_count)) + " rosów online"
            await channel.send(content=msg)
        if(len(ros_przyzy_count) >= players_online_to_ping):
            msg = "Możliwy Przyzy, " + str(len(ros_przyzy_count)) + " rosów online"
            await channel.send(content=msg)
        #if(len(ros_lowka_count) >= players_online_to_ping):
        #    msg = "Możliwa Łowka, " + str(len(ros_lowka_count)) + " rosów online"
        #    await channel.send(content=msg)
        if(len(ros_zoons_count) >= players_online_to_ping):
            msg = "Możliwy(a) Zoons/Łowka, " + str(len(ros_zoons_count)) + " rosów online"
            await channel.send(content=msg)
        #if(len(ros_arcy_count) >= players_online_to_ping):
        #    msg = "Możliwy Arcy, " + str(len(ros_arcy_count)) + " rosów online"
        #    await channel.send(content=msg)
        if(len(ros_renio_count) >= 4):
            msg = "Możliwy Renio, " + str(len(ros_renio_count)) + " rosów online"
            await channel.send(content=msg)
        #if(len(ros_krolik_count) >= players_online_to_ping):
        #    msg = "Możliwy Kr ólik, " + str(len(ros_krolik_count)) + " rosów online"
        #    await channel.send(content=msg)
        #if(len(ros_orla_count) >= players_online_to_ping):
        #    msg = "Możliwa Orla, " + str(len(ros_orla_count)) + " rosów online"
        #    await channel.send(content=msg)
        if(len(west_tanroth_count) >= players_online_to_ping):
            msg = "Możliwy Tanroth, " + str(len(west_tanroth_count)) + " westów online"
            await channel.send(content=msg)
        if(len(west_teza_count) >= players_online_to_ping):
            msg = "Możliwa Teza, " + str(len(west_teza_count)) + " westów online"
            await channel.send(content=msg)
        if(len(west_magua_count) >= players_online_to_ping):
            msg = "Możliwy Magua, " + str(len(west_magua_count)) + " westów online"
            await channel.send(content=msg)
        if(len(west_przyzy_count) >= players_online_to_ping):
            msg = "Możliwy Przyzy, " + str(len(west_przyzy_count)) + " westów online"
            await channel.send(content=msg)
        #if(len(west_lowka_count) >= players_online_to_ping):
        #    msg = "Możliwa Łowka, " + str(len(west_lowka_count)) + " westów online"
        #    await channel.send(content=msg)
        if(len(west_zoons_count) >= players_online_to_ping):
            msg = "Możliwy(a) Zoons/Łowka, " + str(len(west_zoons_count)) + " westów online"
            await channel.send(content=msg)
        #if(len(west_arcy_count) >= players_online_to_ping):
        #    msg = "Możliwy Arcy, " + str(len(west_arcy_count)) + " westów online"
        #    await channel.send(content=msg)
        if(len(west_renio_count) >= players_online_to_ping):
            msg = "Możliwy Renio, " + str(len(west_renio_count)) + " westów online"
            await channel.send(content=msg)
        #if(len(west_krolik_count) >= players_online_to_ping):
        #    msg = "Możliwy Królik, " + str(len(west_krolik_count)) + " westów online"
        #    await channel.send(content=msg)
